...
getIndicators();
setInterval(fetchServerTime, 1000); // Обновляем время каждую секунду

function fetchServerTime() {
  axios
    .get("https://localhost:7045/api/Time")
    .then((response) => {
...
function createIndicator(
  ...
  if (type === "Temperature") {
    createChart(id, name, child, values);
  }
...
function tryNewIndicator(name, value, unit, x, y, type) {
